...
//var tokenLifetime = 1000 * 60 * 60 * 24;

function soapResource() {return '/services/Soap/u/' + settings.sfVersion;}
function resourceRoot(sfVersion) {var vers = sfVersion ? sfVersion : settings.sfVersion; return '/services/data/v' + vers + '/';}
...
function getSession(credentials,callback /* (err,session) */) {
    ...
    if (!credentials.rawResponse.issued_at) {
        return callback(null, emutils.generateCredentialsError('The OAuth acess must be refreshed', 'integration.login.fail.oauthTokenInvalid'));
    }
    if ((Date.now() - credentials.rawResponse.issued_at) > tokenLifetime) {
        return callback(null, emutils.generateCredentialsError('The OAuth access token is expired', 'integration.login.fail.oauthTokenExpired'));
    }
...
Session.prototype.getSalesforceProperties = function(callback, typeNames) {
    var response = {
        status : "SUCCESS",
        results: [],
        warnings: []
    ...
    async.forEachSeries(typeNames,
        function(typeName, acb){
            sess.describeSObject(typeName, function(err, sfDesc) {
                if (err) {
                    response.warnings.push({
                        message: "Error fetching description for " + typeName + ": " + err.toString(),
                        code : 'integration.type.description.not.found',
                        targetType: "CdmWarning"
                    });
                }
                else {
                    var props = [];
                    for (var field in sfDesc.fields) {
                        var name = sfDesc.fields[field].name;
                        if (name != "Id" & name != "SystemModstamp") {
                            props.push({name : name, type : sfDesc.fields[field].type});
                        }
                    }
                    var result = {
                        type : typeName,
...
Session.prototype.getSalesforceTypes = function(callback) {
    restRequest(this,'GET','sobjects/',null,function(err,result) {
        if(err) return callback(err);
        var names = [];
        try {
            result.jsonBody.sobjects.forEach(function(row) {
                names.push(row.name);
            });
        }
        catch (err) {
        ...
        return callback(null, {status : "SUCCESS", results: names})
    });
}
...
function restRequest(session,method,resource,query,callback /* (err,result) */) {
    ...
    var path = resource;
    if(path.charAt(0) != '/') {
        path = resourceRoot(session.sfVersion) + path;
    }
    ...
    resourceRequest(session,method,headers,path,requestBody,function(err,result) {
        ...
        if(result.body.length > 0) {
            try {
                ...
                // But since Salesforce lets non-ASCII characters through, we need to check for them
                result.jsonBody = JSON.parse(removeControlCharacters(result.body));
             } catch(err) {
...
function describeSObject(session,objectName,cb /* (err,metadata) */) {
    ...
    restRequest(session,'GET','sobjects/'+objectName+'/describe/',null,function(err,result) {
        ...
        if(result.jsonBody.name != objectName) {
            console.log(result.status + ' Unexpected reply from Salesforce sobjects/ ');
            console.dir(result);
...
function getFolderId(session,fullPath,callback /* (err,folderId,documentName) */) {
    ...
    var resource = 'query/?q=' + encodeURIComponent(query);
    ...
    restRequest(session,'GET',resource,null,function(err,result) {
        ...
        if(result.status === 200) {
            ...
            callback(new Error('Named folder not found for resource: ' + fullPath + ' status= ' + result.status));
        }
    });
}
    ...
    getFolderId(session,directive.params.name,function(err,folderId,documentName) {
        ...
        restRequest(session,'GET',resource,null,function(err,result) {
            ...
            if(result.status === 200) {
                ...
                callback(new Error('Unsuccessful status code for fetch of ' + documentName + ': ' + result.status));
            }
        });
    });
}
...
function removeControlCharacters(str) {
    ...
    var hasCtlChars = false;
    for (var i = 0; i < str.length; i++) {
        if (isControlCharacter(str.charCodeAt(i))) {
            hasCtlChars = true;
            break;
        }
    }
