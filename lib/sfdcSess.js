...
function getSession(credentials,callback /* (err,session) */) {
    ...
    if(sess) {
        ...
        login(sess,function(err,session) {
            if(!err && session) {
...
function login(session,callback /* (err,session) */) {
    loginWithAPIToken(session.credentials,function(err,host,sessionId) {
        if(err) {
            console.log('Failed to authenticate with Salesforce with given credentials!');
            return callback(err);
        }
        session.host = host;
...
function resourceRequest(session,method,headers,resource,body,callback /* (err,result) */, retry) {
    ...
    if(session) {
        ...
        url = "https://" + session.host + resource;
    } else {
        // This handles the special case of being called for initial authentication
        url = resource;
    }
        ...
        if(session && result.status === 401 && !retry) {
            ...
            session.login(function(err){
                if(err) {
                    ...
                    resourceRequest(session,method,headers,resource,body,callback,true);
                }
            });
        } else {
            callback(err,result);
...
function loginWithAPIToken(credentials,callback /* (err,host,sessionId) */ ) {
    ...
    soapRequest(null,loginResource,loginBody,function(err,result) {
        ...
        if(!result.xmlBody.loginResponse || !result.xmlBody.loginResponse.result) {
            ...
            return callback({
                code:'integration.login.fail.loginResponse',
                message:'Unexpected SOAP response from Salesforce',
        ...
        if(result.passwordExpired == 'true') {
            return callback({
                code:'integration.login.fail.expired',
                message:'Salesforce password expired. Log in to Salesforce.com to update password.'
            });
        }
