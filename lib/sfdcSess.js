...
var soapResource = '/services/Soap/u/24.0';
...
// console.log("session="); console.dir(session);
// console.log("humbug");
    ...
    httpRequest.httpRequest(options,body,function(err,result) {
        ...
        if(session && result.status === 401 && !retry) {
            ...
            if(result.status==204) {
                console.log('Body with 204! ' + result);
                console.dir(result);
            }
...
function loginWithAPIToken(credentials,callback /* (err,host,sessionId) */ ) {
    ...
    var hostString = credentials.host || "login.salesforce.com";
    ...
    soapRequest(session,soapResource,soapBody,function(err,result) {
        if(result.xmlBody.upsertResponse) {
            ...
            if(sfResult && sfResult.errors) {
                ...
                callback(null,result); // TODO: not sure how this result is used
            }
        } else {
...
function restRequest(session,method,resource,query,callback /* (err,result) */) {
    ...
    resourceRequest(session,method,headers,path,requestBody,function(err,result) {
        if(result.status === 400) {
            ...
            console.log(method + ' resource was: ' + resource);
            console.log('Request body was: ' + requestBody);
            var err = new Error('400, MALFORMED REQUEST: ' + resource + ' ' +requestBody);
            callback(err,result.statusCode,null); // body is the error response
        } else {
            if(result.body.length > 0) {
                try {
                    ...
                    console.log('charCodeAt(0) = ' + result.body.charCodeAt(0))
                    var err = new Error('Status=' + result.status);
...
function fetchDocument(session,directive,callback) {
    ...
    getFolderId(session,directive.params.name,function(err,folderId,documentName) {
        ...
        restRequest(session,'GET',resource,null,function(err,result) {
            ...
            if(result.status === 200) {
console.log('Saleforce Document select returned:');
console.dir(result);
