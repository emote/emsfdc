...
function getSession(credentials,callback /* (err,session) */) {
    ...
    var sess = sessionCache[credentials.accessToken];
    if (!sess) {
        sess = sessionCache[credentials.accessToken] = new Session(credentials);
    }

    if ((Date.now - credentials.tokenIssuedAt) > tokenLifetime) {
        callback(null,{
            targetType: 'RestResponse',
            status: 'ERROR',
            errors: [{
                targetType:'CdmError',
                code:'integration.login.fail.oauthTokenExpired',
                message:"The OAuth access token is expired"
            }]
        });
    }
    callback(null,sess);
...
function Session(credentials) {
    ...
    this.tokenIssuedAt = credentials.rawRequest.issuedAt;
    this.sfHostUrl = credentials.rawRequest.instance_url;
...
function resourceRequest(session,method,headers,resource,body,callback /* (err,result) */) {
    var url = "https://" + session.sfHostUrl + resource;
    ...
    options.headers = addAuthHeaders(headers, session.accessToken);
