...
Session.prototype.createSfModel = function(typeNames, callback) {
    var typeDescs = [];

    async.forEachSeries(typeNames,
        function(typeName, acb) {
            describeSObject(this, typName, function(err, result) {
                if (err) {
                    return acb(err);
                }
                typeDescs.push(result);
                acb(null);
            });
        },
        function(err) {
            ...
            var commands = [];
            typeDescs.forEach(function(row) {
                var desc = {
                    "op": "INVOKE",
                    "name": "alterCdmType",
                    "params": {
                ...
                for (var fieldName in row.fields.name) {
                    var field = row.fields.name[fieldName];

                }
            });

            return callback(null, {status : "SUCCESS", results: commands})
        }
    );
}
