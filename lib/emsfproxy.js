...
exports.processDirective = function processDirective(requestData,callback) {
    ...
    if(!(options && options.credentials)) {
        ...
        if (requestData.op == "INVOKE") {
            ...
            else if (requestData.name == "getSalesforceProperties") {
                found = true;
                sfdcSess.getSession(options.credentials,function(err,session) {
                    if(err) return callback(err);
                    if (session.errors && session.errors.length > 0) {
                    ...
                    session.sfVersion = requestData.params.sfVersion;
                    return session.getSalesforceProperties(callback, requestData.params.types);
                });
            }
...
function createSfModel(session, typeDescs, externalSystem, callback) {
        ...
        session.describeSObject(typeDesc.name, function(err, sfDesc) {
            ...
            for (var attrName in attributes) {
                    ...
                    || attr.name == "CreatedById")
                {
                    ...
                    var truncatedName = null;
                    if (attr.name.length > 45) {
                        truncatedName = attr.name.substring(0, 45) + "_" + longNameCount++;
                    }

                    if (attr.name != "Id"  && attr.name != "SystemModstamp")
                    {
                        propList.push({name: truncatedName ? truncatedName : attr.name, type: cdmType});
                    }

                    var targetProp = {name: attr.name, type: cdmType, externalType: attr.type};
                    if (truncatedName) {
                        targetProp.cdmPropName = truncatedName;
                    }

                    targetProperties.push(targetProp);
                }
            }
            ...
            pickListTypes.forEach(function(item) {
    ...
    function processSalesforceTypeContinued(typeDesc, externalSystem, propList, targetProperties, cb) {
        ...
        if (!typeDesc.typeBindingProperties) {
            typeDesc.typeBindingProperties = {};
        }
        for (var prop in typeDesc) {
            switch(prop) {
                case "name":
                case "target":
                case "properties":
                case "typeBindingProperties":
                    break;

                default:
                    typeDesc.typeBindingProperties[prop] = typeDesc[prop];
    ...
    function createBinding(cdmName, sfdcName, bindingProperties, propList, externalSystem) {
        ...
        var renamedProps = {};
        propList.forEach(function(row) {
            if (row.cdmPropName) {
                renamedProps[row.name] = row.cdmPropName;
                delete row.cdmPropName;
            }
        });
        ...
        for(var j=0;j<propList.length;j++) {
            ...
            if(bindProp.name === 'Id') {
                ...
                cdmProp = 'externalTimestamp';
                propType = 'datetime';
            } else if (renamedProps[bindProp.name]) {
                cdmProp = renamedProps[bindProp.name];
            }
