...
function processOperation(session,typeInfo,directive,callback) {
    ...
    if(directive.values) {
        var isUpdate = (directive.op === 'UPDATE');
        directive.values = filterValuesByMetadata(directive.values,typeInfo.fields,isUpdate);
    }
...
function filterValuesByMetadata(values,metadata,isUpdate) {
    var retval = {};
    for(var key in values) {
        if(typeof metadata[key] === 'undefined') {
            console.log('Ignoring update/insert value for a field which is undefined for current user. Field name=' + key);
        } else if(isUpdate && !metadata[key].updateable) {
