...
exports.processDirective = function processDirective(requestData,callback) {
    ...
    if(!(options && options.credentials)) {
        ...
        sfdcSess.loginWithAPIToken(options.credentials,function(err,serverUrl,sessionId) {
            if(err) {
                callback(null,{
                    ...
                    errors: [{
                        targetType:'CdmError',
                        code: (err.code.indexOf('integration.login.fail')===0 ? err.code : 'integration.login.fail'),
                        message:err.message
                    }]
                });
            } else {
        ...
        // SD: Notes on adapting this proxy for use by "schema designer" are labeled "SD"
        // SD: schema designer INVOKE's should not require a session, so they should follow
        // SD: the pattern above and wrap all functions with a call to loginWithAPIToken.
        // SD: Note since none of these use sfdcSess.getSession, they will not result in any
        // SD: cached session data.
        // SD:
        // SD: Suggest three INVOKES: 
        // SD:   getSalesforceObjects -- does same GET in sfsetup.createMappedCustomObjects
        // SD:                           returns only an array or names
        // SD:   getSaleforceMetadata -- wraps sfdcSess.getMetadataForTargetType
        // SD:                           takes only an array of names, and 
        // SD:                           returns field-level metadata for each type
        // SD:   generateSalesforceBindings -- generate script to create/ CDM types, targetTypes, and bindings,
        // SD:                               for a given Salesforce object based on a user
        // SD:                               selected subset of the fields returned by getSaleforceMetadata,
        // SD:                               takes field-level metadata for each bound type 
        // SD:                               (prefer to follow pattern from wsdl support where possible ) 
